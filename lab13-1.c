...
void count_max_link_depth(int recursive_depth) {
	...
	sprintf(previous_file_name, "test%d", recursive_depth);
	sprintf(current_file_name, "test%d", recursive_depth + 1);

	if (symlink(previous_file_name, current_file_name) != 0) {
		printf("Unknown error while creaing symlink. Check your permissions. Terminating.\n");
		exit(-1);
	}
	...
	if ((file_descriptor = open(current_file_name, O_RDWR | O_CREAT, 0666)) < 0) {
		printf("Max recursive link depth is %d.\n",recursive_depth - 1);
